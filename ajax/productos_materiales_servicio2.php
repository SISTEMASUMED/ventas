<?php
	...
	$numero_servicio = ($_GET['numero_servicio']); 
	 echo "<script>console.log('numero de servicio:".$numero_servicio."');</script>";
		...
		if ($numrows>0){
			...
			<div class="table-responsive">
			  <table class="table table-striped">
				<tr  class="info">
					<th class="hidden-xs">Procedimiento</th>
					<th>Referencia</th>
					<th >Producto</th>
					<th >Almacén</th>
					<th class=''>Lote</th>
					<th class=''>Cantidad</th>
					<th class=''>Provedor</th>
					<th class=''>Agregar</th>
				</tr>
				<?php
				while ($row=mysqli_fetch_array($query)){
					...
					<tr >
					<td>
						<select class="" id="procedimiento_<?php echo $id_producto; ?>"style="width:90%;font-size:14px;">
							<option selected="selected" value="">Procedimiento</option>
						<?php $sql_procedimiento2="SELECT * FROM detalle_servicio WHERE numero_servicio = '".$numero_servicio."'"; $query= mysqli_query($con, $sql_procedimiento2);
						...
						while($row_alm=mysqli_fetch_array($query))
								{
								?>
								<option value='<?php echo $row_alm['descripcion'];?>'><?php echo $row_alm['descripcion'];?></option>
								<?php
								}
								?>		
								</select>
						...
						<td>
						<select class="" id="provedor_<?php echo $id_producto; ?>"style="width:130%;font-size:14px;">
							<option selected="selected" value="">Provedor</option>
						<?php $sql_provedor="SELECT * FROM proveedores"; $query= mysqli_query($con, $sql_provedor);
						while($row_provedor=mysqli_fetch_array($query))
								{
								?>
								<option value='<?php echo $row_provedor['nombre'];?>'><?php echo $row_provedor['nombre'];?></option>
								<?php
								}
								?>		
								</select>
